              ...
              <div className="grid gap-5 md:grid-cols-2">
                {banners.slice(0, 2).map((b: any) => (
                  <a
                    key={b.id}
                    href={b.linkUrl || "#"}
                    className="group relative block overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow transition hover:shadow-lg hover:border-[var(--accent)] hover:-translate-y-1"
                  >
                    <img
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      <div className="inline-block bg-black/50 rounded-lg px-3 py-2">
                        <p className="font-bold text-lg sm:text-xl text-white drop-shadow">{b.title}</p>
                        {b.subtitle && (
                          <p className="mt-1.5 text-sm sm:text-base text-white/90">{b.subtitle}</p>
                        )}
                      </div>
                    </div>
                    {b.linkUrl && (
                      <div className="absolute top-4 right-4">
                        <span className="inline-flex items-center gap-1 bg-[var(--accent)] text-white text-xs font-bold px-3 py-1.5 rounded-full shadow transition hover:bg-[var(--accent-2)]">
                          Ver oferta
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-6 gap-2 sm:gap-3">
              {categories.slice(0, 12).map((c: any) => (
                ...
                >
                  ...
                  {/* Si tienes ícono: <CategoryIcon name={c.slug} /> */}
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-5 sm:grid-cols-3 lg:grid-cols-4">
            {items.map((p, idx) => (
              <div className="relative" key={p.id}>
                {idx < 3 && (
                  <span className="absolute top-2 left-2 bg-[var(--accent)] text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow">
                    Nuevo
                  </span>
                )}
                <ProductCard
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center p-6 bg-white rounded-2xl border border-[var(--border)] shadow-sm hover:shadow-md transition">
